...
const cardThemes: Record<string, CardTheme> = {
  ...
  green: {
    ...
    borderHover: "hover:border-[#4ADE80]",
  ...
  red: {
    ...
    borderHover: "hover:border-[#F87171]",
...
const Card = memo(({ icon, title, description, theme, className = "" }: CardProps) => {
  ...
  return (
    <div
      className={`relative group bg-slate-800/50 border border-slate-700 rounded-xl p-4 sm:p-6 lg:p-8 h-64 sm:h-72 lg:h-80 w-full max-w-md sm:max-w-lg lg:max-w-xl overflow-hidden transition-all duration-300 cursor-pointer hover:shadow-2xl ${themeConfig.shadowHover} ${themeConfig.borderHover} ${className}`}
    >
