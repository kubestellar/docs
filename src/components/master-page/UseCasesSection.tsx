...
const cardThemes: Record<string, CardTheme> = {
  yellow: {
    ...
    borderHover: "hover:border-[#FACC15]/60",
    shadowHover: "hover:shadow-yellow-500/30"
  },
  purple: {
    iconBg: "bg-purple-400/20",
    borderHover: "hover:border-[#C084FC]/60",
  ...
  red: {
    ...
    borderHover: "hover:border-[#F87171]/60",
...
const Card = memo(({ icon, title, description, theme, className = "" }: CardProps) => {
  const themeConfig = cardThemes[theme];
  
  return (
    <div
      className={`relative group bg-blue-900/20 backdrop-blur-md border border-blue-700/30 rounded-xl p-4 sm:p-6 lg:p-8 h-64 sm:h-72 lg:h-80 w-full max-w-md sm:max-w-lg lg:max-w-xl overflow-hidden transition-all duration-300 cursor-pointer hover:shadow-2xl hover:bg-blue-900/30 hover:border-blue-600/40 ${themeConfig.shadowHover} ${themeConfig.borderHover} ${className}`}
    >
...
// Move useCases outside component to prevent recreation on every render
const useCases = [
  {
    icon: "cloud",
