...
// Supported locales
const LOCALES = ['en', 'hi', 'ja', 'es', 'de', 'fr', 'it', 'SC', 'zh-TW', 'pt'] as const
...
export default async function Page(props: PageProps) {
  ...
  // Get the base path for links based on project and locale
  const linkBasePath = projectId === 'kubestellar' ? `/${locale}/docs` : `/${locale}/docs/${projectId}`
...
export async function generateStaticParams() {
  ...
  for (const locale of LOCALES) {
    ...
    for (const route of Object.keys(a2aMap.routeMap)) {
      if (route !== '') {
        allParams.push({ locale, slug: ['a2a', ...route.split('/')] })
      }
    }
    ...
    for (const route of Object.keys(kubeflexMap.routeMap)) {
      if (route !== '') {
        allParams.push({ locale, slug: ['kubeflex', ...route.split('/')] })
      }
    }
    ...
    for (const route of Object.keys(multiPluginMap.routeMap)) {
      if (route !== '') {
        allParams.push({ locale, slug: ['multi-plugin', ...route.split('/')] })
      }
    }
    ...
    for (const route of Object.keys(kubectlClaudeMap.routeMap)) {
      if (route !== '') {
        allParams.push({ locale, slug: ['kubectl-claude', ...route.split('/')] })
      }
    }
  }
